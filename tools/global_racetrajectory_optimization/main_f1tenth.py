...
import quadprog_shim as _qp_shim
sys.modules['quadprog'] = _qp_shim
import opt_mintime_traj
import numpy as np
_F1T_TRACK = sys.argv[1] if len(sys.argv) > 1 else "track_20260411_223212"
_F1T_OPT = sys.argv[2] if len(sys.argv) > 2 else "mincurv"
import time
import json
import os
...
file_paths["track_name"] = _F1T_TRACK
...
imp_opts = {"flip_imp_track": False,                # flip imported track to reverse direction
            ...
            "min_track_width": 0.4,                 # [m] enforce floor so optimizer has room on narrow F1Tenth tracks
...
# check dependencies (disabled for F1Tenth fork — we use newer versions)
